...
with open('temporary_file_checking_visits_time','wt') as visits_file:
    for key, value in subjects_dates.items():
        if len(value)==2:
            ...
            visits_file.write("\t first visit: "+str(sorted(value)[0])+"\n")
            visits_file.write("\t second visit: "+str(sorted(value)[1])+"\n")
            visits_file.write("\t difference in years, between the first two visits: "+str(sorted(value)[1]-sorted(value)[0])+"\n")
        elif len(value)==3:  
            visits_file.write("Subject id: "+str(key)+"\n")
            visits_file.write("\t first visit: "+str(sorted(value)[0])+"\n")
            visits_file.write("\t second visit: "+str(sorted(value)[1])+"\n")
            visits_file.write("\t third visit: "+str(sorted(value)[2])+"\n")            
            visits_file.write("\t difference in years, between the first two visits: "+str(sorted(value)[1]-sorted(value)[0])+"\n")
            visits_file.write("\t difference in years, between the second two visits: "+str(sorted(value)[2]-sorted(value)[1])+"\n")
            visits_file.write("\t difference in years, between the first and third visits: "+str(sorted(value)[2]-sorted(value)[0])+"\n")
        else:
            visits_file.write("Subject id: "+str(key)+"\n")
            visits_file.write("\t first visit: "+str(sorted(value)[0])+"\n")
            visits_file.write("\t second visit: "+str(sorted(value)[1])+"\n")
            visits_file.write("\t third visit: "+str(sorted(value)[2])+"\n")
            visits_file.write("\t fourth visit: "+str(sorted(value)[3])+"\n")          
            visits_file.write("\t difference in years, between the first two visits: "+str(sorted(value)[1]-sorted(value)[0])+"\n")
            visits_file.write("\t difference in years, between the second two visits: "+str(sorted(value)[2]-sorted(value)[1])+"\n")
            visits_file.write("\t difference in years, between the first and third visits: "+str(sorted(value)[2]-sorted(value)[0])+"\n")
            visits_file.write("\t difference in years, between the third two visits: "+str(sorted(value)[3]-sorted(value)[2])+"\n")
            visits_file.write("\t difference in years, between the second and fourth visits: "+str(sorted(value)[3]-sorted(value)[1])+"\n")
            visits_file.write("\t difference in years, between the first and fourth visits: "+str(sorted(value)[3]-sorted(value)[0])+"\n")
