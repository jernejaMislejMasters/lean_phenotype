...
#make a log transformed bmi, making sure it is really normal
...
png("Results/distributions/log_bmi_distribution.png")
qqnorm(VIP_data_independant$bmi_norm)
qqline(VIP_data_independant$bmi_norm)
dev.off()
png("Results/distributions/log_sd_bmi_histogram.png")
hist(VIP_data_independant$bmi_sd)
dev.off()
...
VIP_data_independant$POLYsum1_4SD <- POLYsum1
length(VIP_data_independant$POLYsum1_4SD[!is.na(POLYsum1) & (VIP_data_independant$POLYsum1_transformed > 4 | VIP_data_independant$POLYsum1_transformed < -4)])#13
VIP_data_independant$POLYsum1_4SD[!is.na(POLYsum1) & (VIP_data_independant$POLYsum1_transformed > 4 | VIP_data_independant$POLYsum1_transformed < -4)]<-NA
VIP_data_independant$POLYsum1_4SD<-log(VIP_data_independant$POLYsum1_4SD)
VIP_data_independant$POLYsum1_4SD[!is.na(VIP_data_independant$POLYsum1_4SD)]<-(VIP_data_independant$POLYsum1_4SD[!is.na(VIP_data_independant$POLYsum1_4SD)] - 
			mean(VIP_data_independant$POLYsum1_4SD[!is.na(VIP_data_independant$POLYsum1_4SD)]))/(sd(VIP_data_independant$POLYsum1_4SD[!is.na(VIP_data_independant$POLYsum1_4SD)]))

# % of TEI
VIP_data_independant$POLYsum1_ofTEI<-(100*9*POLYsum1)/ensum1
...
VIP_data_independant$POLYsum1_ofTEI_categorized_n[!is.na(POLYsum1)] <-0
VIP_data_independant$POLYsum1_ofTEI_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_ofTEI_transformed >= -1 & VIP_data_independant$POLYsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$POLYsum1_ofTEI_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_ofTEI_transformed >= 0 & VIP_data_independant$POLYsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$POLYsum1_ofTEI_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_ofTEI_transformed >= 1 & VIP_data_independant$POLYsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$POLYsum1_ofTEI_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$POLYsum1_categorized_n[!is.na(POLYsum1)] <-0
VIP_data_independant$POLYsum1_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_transformed >= -1 & VIP_data_independant$POLYsum1_transformed < 0] <- 1
VIP_data_independant$POLYsum1_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_transformed >= 0 & VIP_data_independant$POLYsum1_transformed < 1] <- 2
VIP_data_independant$POLYsum1_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_transformed >= 1 & VIP_data_independant$POLYsum1_transformed < 2] <- 3
VIP_data_independant$POLYsum1_categorized_n[!is.na(POLYsum1) & VIP_data_independant$POLYsum1_transformed >= 2] <- 4
...
VIP_data_independant$MONOsum1_4SD <- MONOsum1
length(VIP_data_independant$MONOsum1_4SD[!is.na(MONOsum1) & (VIP_data_independant$MONOsum1_transformed > 4 | VIP_data_independant$MONOsum1_transformed < -4)])#11
VIP_data_independant$MONOsum1_4SD[!is.na(MONOsum1) & (VIP_data_independant$MONOsum1_transformed > 4 | VIP_data_independant$MONOsum1_transformed < -4)]<-NA
VIP_data_independant$MONOsum1_4SD<-log(VIP_data_independant$MONOsum1_4SD)
VIP_data_independant$MONOsum1_4SD[!is.na(VIP_data_independant$MONOsum1_4SD)]<-(VIP_data_independant$MONOsum1_4SD[!is.na(VIP_data_independant$MONOsum1_4SD)] - 
			mean(VIP_data_independant$MONOsum1_4SD[!is.na(VIP_data_independant$MONOsum1_4SD)]))/(sd(VIP_data_independant$MONOsum1_4SD[!is.na(VIP_data_independant$MONOsum1_4SD)]))

# % of TEI
VIP_data_independant$MONOsum1_ofTEI<-(100*9*MONOsum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$MONOsum1_ofTEI_categorized_n[!is.na(MONOsum1)] <-0
VIP_data_independant$MONOsum1_ofTEI_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_ofTEI_transformed >= -1 & VIP_data_independant$MONOsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$MONOsum1_ofTEI_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_ofTEI_transformed >= 0 & VIP_data_independant$MONOsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$MONOsum1_ofTEI_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_ofTEI_transformed >= 1 & VIP_data_independant$MONOsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$MONOsum1_ofTEI_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$MONOsum1_categorized_n[!is.na(MONOsum1)] <-0
VIP_data_independant$MONOsum1_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_transformed >= -1 & VIP_data_independant$MONOsum1_transformed < 0] <- 1
VIP_data_independant$MONOsum1_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_transformed >= 0 & VIP_data_independant$MONOsum1_transformed < 1] <- 2
VIP_data_independant$MONOsum1_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_transformed >= 1 & VIP_data_independant$MONOsum1_transformed < 2] <- 3
VIP_data_independant$MONOsum1_categorized_n[!is.na(MONOsum1) & VIP_data_independant$MONOsum1_transformed >= 2] <- 4
...
VIP_data_independant$mfetsum1_4SD <- mfetsum1
length(VIP_data_independant$mfetsum1_4SD[!is.na(mfetsum1) & (VIP_data_independant$mfetsum1_transformed > 4 | VIP_data_independant$mfetsum1_transformed < -4)])#12
VIP_data_independant$mfetsum1_4SD[!is.na(mfetsum1) & (VIP_data_independant$mfetsum1_transformed > 4 | VIP_data_independant$mfetsum1_transformed < -4)]<-NA
VIP_data_independant$mfetsum1_4SD<-log(VIP_data_independant$mfetsum1_4SD)
VIP_data_independant$mfetsum1_4SD[!is.na(VIP_data_independant$mfetsum1_4SD)]<-(VIP_data_independant$mfetsum1_4SD[!is.na(VIP_data_independant$mfetsum1_4SD)] - 
			mean(VIP_data_independant$mfetsum1_4SD[!is.na(VIP_data_independant$mfetsum1_4SD)]))/(sd(VIP_data_independant$mfetsum1_4SD[!is.na(VIP_data_independant$mfetsum1_4SD)]))

# % of TEI
VIP_data_independant$mfetsum1_ofTEI<-(100*9*mfetsum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$mfetsum1_ofTEI_categorized_n[!is.na(mfetsum1)] <-0
VIP_data_independant$mfetsum1_ofTEI_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_ofTEI_transformed >= -1 & VIP_data_independant$mfetsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$mfetsum1_ofTEI_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_ofTEI_transformed >= 0 & VIP_data_independant$mfetsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$mfetsum1_ofTEI_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_ofTEI_transformed >= 1 & VIP_data_independant$mfetsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$mfetsum1_ofTEI_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$mfetsum1_categorized_n[!is.na(mfetsum1)] <-0
VIP_data_independant$mfetsum1_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_transformed >= -1 & VIP_data_independant$mfetsum1_transformed < 0] <- 1
VIP_data_independant$mfetsum1_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_transformed >= 0 & VIP_data_independant$mfetsum1_transformed < 1] <- 2
VIP_data_independant$mfetsum1_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_transformed >= 1 & VIP_data_independant$mfetsum1_transformed < 2] <- 3
VIP_data_independant$mfetsum1_categorized_n[!is.na(mfetsum1) & VIP_data_independant$mfetsum1_transformed >= 2] <- 4
...
VIP_data_independant$fettsum1_4SD <- fettsum1
length(VIP_data_independant$fettsum1_4SD[!is.na(fettsum1) & (VIP_data_independant$fettsum1_transformed > 4 | VIP_data_independant$fettsum1_transformed < -4)])#9
VIP_data_independant$fettsum1_4SD[!is.na(fettsum1) & (VIP_data_independant$fettsum1_transformed > 4 | VIP_data_independant$fettsum1_transformed < -4)]<-NA
VIP_data_independant$fettsum1_4SD<-log(VIP_data_independant$fettsum1_4SD)
VIP_data_independant$fettsum1_4SD[!is.na(VIP_data_independant$fettsum1_4SD)]<-(VIP_data_independant$fettsum1_4SD[!is.na(VIP_data_independant$fettsum1_4SD)] - 
			mean(VIP_data_independant$fettsum1_4SD[!is.na(VIP_data_independant$fettsum1_4SD)]))/(sd(VIP_data_independant$fettsum1_4SD[!is.na(VIP_data_independant$fettsum1_4SD)]))

# % of TEI
VIP_data_independant$fettsum1_ofTEI<-(100*9*fettsum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$fettsum1_ofTEI_categorized_n[!is.na(fettsum1)] <-0
VIP_data_independant$fettsum1_ofTEI_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_ofTEI_transformed >= -1 & VIP_data_independant$fettsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$fettsum1_ofTEI_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_ofTEI_transformed >= 0 & VIP_data_independant$fettsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$fettsum1_ofTEI_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_ofTEI_transformed >= 1 & VIP_data_independant$fettsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$fettsum1_ofTEI_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$fettsum1_categorized_n[!is.na(fettsum1)] <-0
VIP_data_independant$fettsum1_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_transformed >= -1 & VIP_data_independant$fettsum1_transformed < 0] <- 1
VIP_data_independant$fettsum1_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_transformed >= 0 & VIP_data_independant$fettsum1_transformed < 1] <- 2
VIP_data_independant$fettsum1_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_transformed >= 1 & VIP_data_independant$fettsum1_transformed < 2] <- 3
VIP_data_independant$fettsum1_categorized_n[!is.na(fettsum1) & VIP_data_independant$fettsum1_transformed >= 2] <- 4
...
VIP_data_independant$FA_4SD <- FA
length(VIP_data_independant$FA_4SD[!is.na(FA) & (VIP_data_independant$FA_transformed > 4 | VIP_data_independant$FA_transformed < -4)])#15
VIP_data_independant$FA_4SD[!is.na(FA) & (VIP_data_independant$FA_transformed > 4 | VIP_data_independant$FA_transformed < -4)]<-NA
VIP_data_independant$FA_4SD<-log(VIP_data_independant$FA_4SD)
VIP_data_independant$FA_4SD[!is.na(VIP_data_independant$FA_4SD)]<-(VIP_data_independant$FA_4SD[!is.na(VIP_data_independant$FA_4SD)] - 
			mean(VIP_data_independant$FA_4SD[!is.na(VIP_data_independant$FA_4SD)]))/(sd(VIP_data_independant$FA_4SD[!is.na(VIP_data_independant$FA_4SD)]))

# % of TEI
VIP_data_independant$FA_ofTEI<-(100*9*FA)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$FA_ofTEI_categorized_n[!is.na(FA)] <-0
VIP_data_independant$FA_ofTEI_categorized_n[!is.na(FA) & VIP_data_independant$FA_ofTEI_transformed >= -1 & VIP_data_independant$FA_ofTEI_transformed < 0] <- 1
VIP_data_independant$FA_ofTEI_categorized_n[!is.na(FA) & VIP_data_independant$FA_ofTEI_transformed >= 0 & VIP_data_independant$FA_ofTEI_transformed < 1] <- 2
VIP_data_independant$FA_ofTEI_categorized_n[!is.na(FA) & VIP_data_independant$FA_ofTEI_transformed >= 1 & VIP_data_independant$FA_ofTEI_transformed < 2] <- 3
VIP_data_independant$FA_ofTEI_categorized_n[!is.na(FA) & VIP_data_independant$FA_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$FA_categorized_n[!is.na(FA)] <-0
VIP_data_independant$FA_categorized_n[!is.na(FA) & VIP_data_independant$FA_transformed >= -1 & VIP_data_independant$FA_transformed < 0] <- 1
VIP_data_independant$FA_categorized_n[!is.na(FA) & VIP_data_independant$FA_transformed >= 0 & VIP_data_independant$FA_transformed < 1] <- 2
VIP_data_independant$FA_categorized_n[!is.na(FA) & VIP_data_independant$FA_transformed >= 1 & VIP_data_independant$FA_transformed < 2] <- 3
VIP_data_independant$FA_categorized_n[!is.na(FA) & VIP_data_independant$FA_transformed >= 2] <- 4
...
VIP_data_independant$kolhsum1_4SD <- kolhsum1
length(VIP_data_independant$kolhsum1_4SD[!is.na(kolhsum1) & (VIP_data_independant$kolhsum1_transformed > 4 | VIP_data_independant$kolhsum1_transformed < -4)])#47
VIP_data_independant$kolhsum1_4SD[!is.na(kolhsum1) & (VIP_data_independant$kolhsum1_transformed > 4 | VIP_data_independant$kolhsum1_transformed < -4)]<-NA
VIP_data_independant$kolhsum1_4SD<-log(VIP_data_independant$kolhsum1_4SD)
VIP_data_independant$kolhsum1_4SD[!is.na(VIP_data_independant$kolhsum1_4SD)]<-(VIP_data_independant$kolhsum1_4SD[!is.na(VIP_data_independant$kolhsum1_4SD)] - 
			mean(VIP_data_independant$kolhsum1_4SD[!is.na(VIP_data_independant$kolhsum1_4SD)]))/(sd(VIP_data_independant$kolhsum1_4SD[!is.na(VIP_data_independant$kolhsum1_4SD)]))

# % of TEI
VIP_data_independant$kolhsum1_ofTEI<-(100*4*kolhsum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$kolhsum1_ofTEI_categorized_n[!is.na(kolhsum1)] <-0
VIP_data_independant$kolhsum1_ofTEI_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_ofTEI_transformed >= -1 & VIP_data_independant$kolhsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$kolhsum1_ofTEI_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_ofTEI_transformed >= 0 & VIP_data_independant$kolhsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$kolhsum1_ofTEI_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_ofTEI_transformed >= 1 & VIP_data_independant$kolhsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$kolhsum1_ofTEI_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$kolhsum1_categorized_n[!is.na(kolhsum1)] <-0
VIP_data_independant$kolhsum1_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_transformed >= -1 & VIP_data_independant$kolhsum1_transformed < 0] <- 1
VIP_data_independant$kolhsum1_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_transformed >= 0 & VIP_data_independant$kolhsum1_transformed < 1] <- 2
VIP_data_independant$kolhsum1_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_transformed >= 1 & VIP_data_independant$kolhsum1_transformed < 2] <- 3
VIP_data_independant$kolhsum1_categorized_n[!is.na(kolhsum1) & VIP_data_independant$kolhsum1_transformed >= 2] <- 4
...
VIP_data_independant$sacksum1_4SD <- sacksum1
length(VIP_data_independant$sacksum1_4SD[!is.na(sacksum1) & (VIP_data_independant$sacksum1_transformed > 4 | VIP_data_independant$sacksum1_transformed < -4)])#37
VIP_data_independant$sacksum1_4SD[!is.na(sacksum1) & (VIP_data_independant$sacksum1_transformed > 4 | VIP_data_independant$sacksum1_transformed < -4)]<-NA
VIP_data_independant$sacksum1_4SD<-log(VIP_data_independant$sacksum1_4SD)
VIP_data_independant$sacksum1_4SD[!is.na(VIP_data_independant$sacksum1_4SD)]<-(VIP_data_independant$sacksum1_4SD[!is.na(VIP_data_independant$sacksum1_4SD)] - 
			mean(VIP_data_independant$sacksum1_4SD[!is.na(VIP_data_independant$sacksum1_4SD)]))/(sd(VIP_data_independant$sacksum1_4SD[!is.na(VIP_data_independant$sacksum1_4SD)]))

# % of TEI
VIP_data_independant$sacksum1_ofTEI<-(100*4*sacksum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$sacksum1_ofTEI_categorized_n[!is.na(sacksum1)] <-0
VIP_data_independant$sacksum1_ofTEI_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_ofTEI_transformed >= -1 & VIP_data_independant$sacksum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$sacksum1_ofTEI_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_ofTEI_transformed >= 0 & VIP_data_independant$sacksum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$sacksum1_ofTEI_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_ofTEI_transformed >= 1 & VIP_data_independant$sacksum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$sacksum1_ofTEI_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$sacksum1_categorized_n[!is.na(sacksum1)] <-0
VIP_data_independant$sacksum1_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_transformed >= -1 & VIP_data_independant$sacksum1_transformed < 0] <- 1
VIP_data_independant$sacksum1_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_transformed >= 0 & VIP_data_independant$sacksum1_transformed < 1] <- 2
VIP_data_independant$sacksum1_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_transformed >= 1 & VIP_data_independant$sacksum1_transformed < 2] <- 3
VIP_data_independant$sacksum1_categorized_n[!is.na(sacksum1) & VIP_data_independant$sacksum1_transformed >= 2] <- 4
...
VIP_data_independant$protsum1_4SD <- protsum1
length(VIP_data_independant$protsum1_4SD[!is.na(protsum1) & (VIP_data_independant$protsum1_transformed > 4 | VIP_data_independant$protsum1_transformed < -4)])#5
VIP_data_independant$protsum1_4SD[!is.na(protsum1) & (VIP_data_independant$protsum1_transformed > 4 | VIP_data_independant$protsum1_transformed < -4)]<-NA
VIP_data_independant$protsum1_4SD<-log(VIP_data_independant$protsum1_4SD)
VIP_data_independant$protsum1_4SD[!is.na(VIP_data_independant$protsum1_4SD)]<-(VIP_data_independant$protsum1_4SD[!is.na(VIP_data_independant$protsum1_4SD)] - 
			mean(VIP_data_independant$protsum1_4SD[!is.na(VIP_data_independant$protsum1_4SD)]))/(sd(VIP_data_independant$protsum1_4SD[!is.na(VIP_data_independant$protsum1_4SD)]))

# % of TEI
VIP_data_independant$protsum1_ofTEI<-(100*4*protsum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$protsum1_ofTEI_categorized_n[!is.na(protsum1)] <-0
VIP_data_independant$protsum1_ofTEI_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_ofTEI_transformed >= -1 & VIP_data_independant$protsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$protsum1_ofTEI_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_ofTEI_transformed >= 0 & VIP_data_independant$protsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$protsum1_ofTEI_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_ofTEI_transformed >= 1 & VIP_data_independant$protsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$protsum1_ofTEI_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$protsum1_categorized_n[!is.na(protsum1)] <-0
VIP_data_independant$protsum1_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_transformed >= -1 & VIP_data_independant$protsum1_transformed < 0] <- 1
VIP_data_independant$protsum1_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_transformed >= 0 & VIP_data_independant$protsum1_transformed < 1] <- 2
VIP_data_independant$protsum1_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_transformed >= 1 & VIP_data_independant$protsum1_transformed < 2] <- 3
VIP_data_independant$protsum1_categorized_n[!is.na(protsum1) & VIP_data_independant$protsum1_transformed >= 2] <- 4
...
VIP_data_independant$fibesum1_4SD <- fibesum1
length(VIP_data_independant$fibesum1_4SD[!is.na(fibesum1) & (VIP_data_independant$fibesum1_transformed > 4 | VIP_data_independant$fibesum1_transformed < -4)])#24
VIP_data_independant$fibesum1_4SD[!is.na(fibesum1) & (VIP_data_independant$fibesum1_transformed > 4 | VIP_data_independant$fibesum1_transformed < -4)]<-NA
VIP_data_independant$fibesum1_4SD<-log(VIP_data_independant$fibesum1_4SD)
VIP_data_independant$fibesum1_4SD[!is.na(VIP_data_independant$fibesum1_4SD)]<-(VIP_data_independant$fibesum1_4SD[!is.na(VIP_data_independant$fibesum1_4SD)] - 
			mean(VIP_data_independant$fibesum1_4SD[!is.na(VIP_data_independant$fibesum1_4SD)]))/(sd(VIP_data_independant$fibesum1_4SD[!is.na(VIP_data_independant$fibesum1_4SD)]))

# % of TEI
VIP_data_independant$fibesum1_ofTEI<-(100*4*fibesum1)/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$fibesum1_ofTEI_categorized_n[!is.na(fibesum1)] <-0
VIP_data_independant$fibesum1_ofTEI_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_ofTEI_transformed >= -1 & VIP_data_independant$fibesum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$fibesum1_ofTEI_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_ofTEI_transformed >= 0 & VIP_data_independant$fibesum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$fibesum1_ofTEI_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_ofTEI_transformed >= 1 & VIP_data_independant$fibesum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$fibesum1_ofTEI_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$fibesum1_categorized_n[!is.na(fibesum1)] <-0
VIP_data_independant$fibesum1_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_transformed >= -1 & VIP_data_independant$fibesum1_transformed < 0] <- 1
VIP_data_independant$fibesum1_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_transformed >= 0 & VIP_data_independant$fibesum1_transformed < 1] <- 2
VIP_data_independant$fibesum1_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_transformed >= 1 & VIP_data_independant$fibesum1_transformed < 2] <- 3
VIP_data_independant$fibesum1_categorized_n[!is.na(fibesum1) & VIP_data_independant$fibesum1_transformed >= 2] <- 4
...
#continuous log transformed and standardized
VIP_data_independant$NATRsum1_transformed<-log(NATRsum1/1000)
...
VIP_data_independant$NATRsum1_4SD <- NATRsum1
length(VIP_data_independant$NATRsum1_4SD[!is.na(NATRsum1) & (VIP_data_independant$NATRsum1_transformed > 4 | VIP_data_independant$NATRsum1_transformed < -4)])#7
VIP_data_independant$NATRsum1_4SD[!is.na(NATRsum1) & (VIP_data_independant$NATRsum1_transformed > 4 | VIP_data_independant$NATRsum1_transformed < -4)]<-NA
VIP_data_independant$NATRsum1_4SD<-log(VIP_data_independant$NATRsum1_4SD)
VIP_data_independant$NATRsum1_4SD[!is.na(VIP_data_independant$NATRsum1_4SD)]<-(VIP_data_independant$NATRsum1_4SD[!is.na(VIP_data_independant$NATRsum1_4SD)] - 
			mean(VIP_data_independant$NATRsum1_4SD[!is.na(VIP_data_independant$NATRsum1_4SD)]))/(sd(VIP_data_independant$NATRsum1_4SD[!is.na(VIP_data_independant$NATRsum1_4SD)]))

# % of TEI
VIP_data_independant$NATRsum1_ofTEI<-(100*4*(NATRsum1/1000))/ensum1
...
# categorizing the % of TEI based on location in standard normal distribution
VIP_data_independant$NATRsum1_ofTEI_categorized_n[!is.na(NATRsum1)] <-0
VIP_data_independant$NATRsum1_ofTEI_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_ofTEI_transformed >= -1 & VIP_data_independant$NATRsum1_ofTEI_transformed < 0] <- 1
VIP_data_independant$NATRsum1_ofTEI_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_ofTEI_transformed >= 0 & VIP_data_independant$NATRsum1_ofTEI_transformed < 1] <- 2
VIP_data_independant$NATRsum1_ofTEI_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_ofTEI_transformed >= 1 & VIP_data_independant$NATRsum1_ofTEI_transformed < 2] <- 3
VIP_data_independant$NATRsum1_ofTEI_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_ofTEI_transformed >= 2] <- 4

# categorizing raw based on location in standard normal distribution
VIP_data_independant$NATRsum1_categorized_n[!is.na(NATRsum1)] <-0
VIP_data_independant$NATRsum1_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_transformed >= -1 & VIP_data_independant$NATRsum1_transformed < 0] <- 1
VIP_data_independant$NATRsum1_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_transformed >= 0 & VIP_data_independant$NATRsum1_transformed < 1] <- 2
VIP_data_independant$NATRsum1_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_transformed >= 1 & VIP_data_independant$NATRsum1_transformed < 2] <- 3
VIP_data_independant$NATRsum1_categorized_n[!is.na(NATRsum1) & VIP_data_independant$NATRsum1_transformed >= 2] <- 4

## ensum1 is raw, ensum1_transformed is raw log tranformed and standardized
#
##continuous log transformed and standardized
#VIP_data_independant$ensum1_transformed<-log(ensum1)
#VIP_data_independant$ensum1_transformed[!is.na(ensum1)]<-(VIP_data_independant$ensum1_transformed[!is.na(ensum1)]-
#			mean(VIP_data_independant$ensum1_transformed[!is.na(ensum1)]))/sd(VIP_data_independant$ensum1_transformed[!is.na(ensum1)])
...
rcorr( cbind(bmi_4SD,POLYsum1_4SD,MONOsum1_4SD,mfetsum1_4SD,fettsum1_4SD,sacksum1_4SD,kolhsum1_4SD,FA_4SD,protsum1_4SD,fibesum1_4SD,NATRsum1_4SD),type="pearson")

# check all the paired correlations for bmi and filtered nutrients
rcorr( cbind(bmi,POLYsum1_4SD,MONOsum1_4SD,mfetsum1_4SD,fettsum1_4SD,sacksum1_4SD,kolhsum1_4SD,FA_4SD,protsum1_4SD,fibesum1_4SD,NATRsum1_4SD),type="pearson")
...
##filtered bmi SD3
#associations<-glm(bmi_3SD~age + agesq + gender_factor + year + ffq_factor + POLYsum1_transformed + MONOsum1_transformed + mfetsum1_transformed + fettsum1_transformed + sacksum1_transformed
#				+ kolhsum1_transformed + FA_transformed + protsum1_transformed + fibesum1_transformed + NATRsum1_transformed, family = gaussian(link = "identity"))
#
#summary(associations)
...
#filtered bmi SD4 and filtered other variables
associations<-glm(bmi_4SD~age + agesq + gender_factor + year + ffq_factor + POLYsum1_4SD + MONOsum1_4SD + mfetsum1_4SD + fettsum1_4SD + sacksum1_4SD
				+ kolhsum1_4SD + FA_4SD + protsum1_4SD + fibesum1_4SD + NATRsum1_4SD, family = gaussian(link = "identity"))

summary(associations)

#filtered just variables
associations<-glm(bmi_sd~age + agesq + gender_factor + year + ffq_factor + POLYsum1_4SD + MONOsum1_4SD + mfetsum1_4SD + fettsum1_4SD + sacksum1_4SD
				+ kolhsum1_4SD + FA_4SD + protsum1_4SD + fibesum1_4SD + NATRsum1_4SD, family = gaussian(link = "identity"))

summary(associations)


all_variables<-c("ensum1_transformed", "POLYsum1_transformed","MONOsum1_transformed","mfetsum1_transformed","fettsum1_transformed","sacksum1_transformed",
